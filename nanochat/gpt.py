...
class GPTConfig:
    ...
    # Characters: L=long (full context), S=explicit local window.
...
class GPT(nn.Module):
    ...
    def _compute_window_sizes(self, config):
        ...
        Pattern string is tiled across layers. Checkpoint-compatible defaults retain
        ...
        short_window = configured_window if configured_window is not None else -(-long_window // 4 // 128) * 128
        assert 0 < short_window <= long_window, "sliding_window must be in [1, sequence_len]"
        char_to_window = {
            "L": (long_window, 0),
            "S": (short_window, 0),
        }
