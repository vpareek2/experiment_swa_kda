...
#pragma unroll
      for (int key = lane; key < kDim; key += warpSize) {
        decay[key] = expf(__half2float(prefix_g[
...
__global__ void nanochat_kda_wy_backward_group_boundary_wmma_c64_kernel(
  ...
  for (int local_chunk = 0; local_chunk < group_chunks; ++local_chunk) {
    ...
    if (warp < kChunk / kMatrix) {
      ...
      for (int key_start = 0; key_start < kDim; key_start += kMatrix) {
        for (int element = lane; element < kTileElements;
          ...
          operand_a[warp * kTileElements + element] = W_bf16[offset];
        }
        __syncwarp();
        MatrixA a_fragment;
        wmma::load_matrix_sync(
    ...
    for (int index = threadIdx.x; index < kDim * kValueTile;
      ...
      const int key = index / kValueTile;
      local_state[index] =
          expf(__half2float(prefix_g[chunk_vector_index(n, kChunk - 1, key)])) *
...
#pragma unroll
        for (int element = 0; element < qg_do.num_elements; ++element) {
          ...
          const float decay = expf(__half2float(prefix_g[chunk_vector_index(
...
void nanochat_kda_wy_backward_fused_boundary_register_dh_group_c64_kernel(
  ...
  __nv_bfloat16* boundary_e_panel = boundary_w_panel + kChunk * 136;
  __nv_bfloat16* boundary_z_shared = boundary_e_panel + kDim * 72;
  float* boundary_decay = reinterpret_cast<float*>(
  ...
  if (threadIdx.x < 32) {
    ...
    #pragma unroll
      ...
      for (int local_chunk = 0; local_chunk < group_chunks; ++local_chunk) {
        ...
        for (int vector = boundary_thread;
          ...
          const int column_vector = vector - row * (kDim / 8);
          __pipeline_memcpy_async(
              boundary_w_panel + row * kWPitch + column_vector * 8,
              w_source + row * kDim + column_vector * 8, 16);
        }
        const __nv_bfloat16* e_source =
            E_transposed + static_cast<int64_t>(group_n) * kDim * kChunk;
        for (int vector = boundary_thread;
          ...
          const int source_vector = vector - key * (kChunk / 8);
          __pipeline_memcpy_async(
              boundary_e_panel + key * kEPitch + source_vector * 8,
              e_source + key * kChunk + source_vector * 8, 16);
        }
        __pipeline_commit();
    ...
    #pragma unroll
          for (int row_tile = 0; row_tile < kRowTiles; ++row_tile) {
            ...
            uint32_t a0, a1, a2, a3;
            wy_ldmatrix_a_m16k16(
                boundary_w_panel + row_start * kWPitch + key_start, kWPitch,
                a0, a1, a2, a3);
    #pragma unroll
            for (int value_half = 0; value_half < 2; ++value_half) {
              const uint32_t b0 = wy_pack_bf16_pair(
    ...
    #pragma unroll
          for (int value_half = 0; value_half < 2; ++value_half) {
            ...
            const __nv_bfloat16 bz10 = __float2bfloat16_rn(z10);
            const __nv_bfloat16 bz11 = __float2bfloat16_rn(z11);
            boundary_z_shared[value0 * kZPitch + row0] = bz00;
            boundary_z_shared[value1 * kZPitch + row0] = bz01;
            boundary_z_shared[value0 * kZPitch + row1] = bz10;
            boundary_z_shared[value1 * kZPitch + row1] = bz11;
            z_history[u00] = bz00;
            z_history[u01] = bz01;
            z_history[u10] = bz10;
            z_history[u11] = bz11;
    ...
    #pragma unroll
        for (int key = lane; key < kDim; key += warpSize) {
          boundary_decay[key] = expf(__half2float(prefix_g[
              chunk_vector_index(n, kChunk - 1, key)]));
        }
    ...
    #pragma unroll
          for (int key_tile = 0; key_tile < kKeyTiles; ++key_tile) {
            ...
            uint32_t a0, a1, a2, a3;
            wy_ldmatrix_a_m16k16(
                boundary_e_panel + key_start * kEPitch + source_start, kEPitch,
                a0, a1, a2, a3);
    #pragma unroll
            for (int value_half = 0; value_half < 2; ++value_half) {
              const int value_base = value_half * kMmaValues;
              ...
              // four shared-memory banks.
              const uint32_t zb0 = *reinterpret_cast<const uint32_t*>(
                  boundary_z_shared + value * kZPitch + source0);
              const uint32_t zb1 = *reinterpret_cast<const uint32_t*>(
                  boundary_z_shared + value * kZPitch + source2);
              wy_mma_bf16_m16n8k16(
                  a0, a1, a2, a3, zb0, zb1,
                  next[key_tile][value_half][0],
                  next[key_tile][value_half][1],
        ...
        // Convert each accumulator fragment to the B-register lane layout for the
        // next chunk, then apply the same FP32 boundary_decay/update order as attempt204.
    #pragma unroll
        for (int key_tile = 0; key_tile < kKeyTiles; ++key_tile) {
    #pragma unroll
          for (int value_half = 0; value_half < 2; ++value_half) {
            ...
            const int key2 = key_start + row2;
            const int key3 = key_start + row3;
            const float decay0 = boundary_decay[key0];
            const float decay1 = boundary_decay[key1];
            const float decay2 = boundary_decay[key2];
            const float decay3 = boundary_decay[key3];
            state[key_tile][value_half][0] =
                decay0 * state[key_tile][value_half][0] + next0;
            state[key_tile][value_half][1] =
                decay1 * state[key_tile][value_half][1] + next1;
    ...
    const int register_thread = threadIdx.x - 32;
      ...
      const int recurrence = owner / (kDim / kValueStrip);
      const int value_start = value_strip * kValueStrip;
      const int warp = register_thread / warpSize;
      const int lane = register_thread % warpSize;
    ...
    #pragma unroll
          for (int value_tile = 0; value_tile < kValueTiles; ++value_tile) {
            wmma::store_matrix_sync(
                register_dh_shared + key_tile * kTile * kValueStrip + value_tile * kTile,
                dh_fragment[owned_key][value_tile], kValueStrip,
                wmma::mem_row_major);
          }
        }
        nanochat_kda_wy_register_dh_barrier();
        for (int index = register_thread; index < kDim * kValueStrip;
             index += 128) {
          const int key = index / kValueStrip;
          const int value = index - key * kValueStrip;
          const __nv_bfloat16 rounded = __float2bfloat16_rn(register_dh_shared[index]);
          register_dh_bf16[index] = rounded;
          dstate_next_history[
              (static_cast<int64_t>(group_n) * kDim + key) * kDim +
              value_start + value] = rounded;
        }
    ...
    #pragma unroll
          for (int value_tile = 0; value_tile < kValueTiles; ++value_tile) {
            ...
            for (int key_start = 0; key_start < kDim; key_start += kTile) {
              ...
              wmma::load_matrix_sync(
                  dh_operand,
                  register_dh_bf16 + key_start * kValueStrip + local_value_start,
                  kValueStrip);
              wmma::mma_sync(kg_dh, kg_operand, dh_operand, kg_dh);
            }
    ...
    #pragma unroll
          for (int value_tile = 0; value_tile < kValueTiles; ++value_tile) {
            ...
            for (int source_start = 0; source_start < kChunk;
              ...
              wmma::load_matrix_sync(
                  w_operand,
                  W_bf16 + (static_cast<int64_t>(group_n) * kChunk + source_start) *
                          kDim + key_start,
                  kDim);
              wmma::load_matrix_sync(
                  dz_operand,
                  register_dz_shared + source_start * kValueStrip + local_value_start,
                  kValueStrip);
              wmma::mma_sync(w_dz, w_operand, dz_operand, w_dz);
            }
    #pragma unroll
...
__global__ void nanochat_kda_wy_backward_complete_four_warp_vjp_c64_kernel(
  ...
  struct KeyPipelineStorage {
    float products[2][3 * kStripElements];
    __nv_bfloat16 left[kStripElements];
  };
  struct FinalTransformStorage {
    __nv_bfloat16 left[kMatrixElements];
    float result[kMatrixElements];
  };
  union SharedStorage {
...
#pragma unroll
  ...
  // Publish the completed dZ P^T / dP split before repurposing all helper
  // warps as key-product producers.
  __syncthreads();

  // Warm key strip zero, then pipeline each following strip: helpers 4--7
  // produce dR/dE/dW for the next strip while owners 0--3 consume the prior
  // buffer. The full-CTA boundary publishes that buffer and makes the other
  // one reusable; owner-only dependencies retain their 128-thread barrier.
  constexpr int kKeyStrips = kDim / kStrip;
  for (int pipeline_stage = 0; pipeline_stage <= kKeyStrips;
       ++pipeline_stage) {
    if (warp >= 4 && pipeline_stage < kKeyStrips) {
      const int produced_key_start = pipeline_stage * kStrip;
      float* produced =
          shared.key.products[pipeline_stage & 1];
      ...
      for (int value_start = 0; value_start < kDim;
        ...
        wmma::load_matrix_sync(
            h_operand,
            state_history +
                (static_cast<int64_t>(local_n) * kDim +
                 produced_key_start) * kDim + value_start,
            kDim);
        wmma::load_matrix_sync(
            dh_operand,
            dstate_history +
      ...
      wmma::store_matrix_sync(
          produced + row_start * kStrip, dR_fragment, kStrip,
          wmma::mem_row_major);
      wmma::store_matrix_sync(
          produced + kStripElements + row_start * kStrip,
          dE_fragment, kStrip, wmma::mem_row_major);
      wmma::store_matrix_sync(
          produced + 2 * kStripElements + row_start * kStrip,
          dW_fragment, kStrip, wmma::mem_row_major);
    }

    if (warp < 4 && pipeline_stage > 0) {
      const int key_strip = pipeline_stage - 1;
      const int key_start = key_strip * kStrip;
      const float* consumed = shared.key.products[key_strip & 1];
      ...
      for (int index = threadIdx.x; index < kStripElements;
        ...
        shared.key.left[index] = __float2bfloat16_rn(
            -consumed[2 * kStripElements + index]);
      }
      __barrier_sync_count(1, 128);

      MatrixARow dw_operand;
      wmma::load_matrix_sync(
          dw_operand, shared.key.left + row_start * kStrip, kStrip);
#pragma unroll
      for (int column_tile = 0; column_tile < 4; ++column_tile) {
      ...
      for (int inner_start = 0; inner_start < kChunk;
        ...
        wmma::load_matrix_sync(
            dw_inner_fragment,
            shared.key.left + inner_start * kStrip, kStrip);
        wmma::mma_sync(
            dQ_fragment, t_fragment, dw_inner_fragment, dQ_fragment);
      }

      // Consume the same fixed SM121 accumulator layout and preserve the
      // original direct-update order for each ascending key strip.
      float beta_lower = 0.0f;
      float beta_upper = 0.0f;
#pragma unroll
      for (int element = 0; element < dQ_fragment.num_elements; ++element) {
        ...
        const float exp_g = expf(__half2float(prefix_g[source]));
        const float beta_row =
            beta[static_cast<int64_t>(n) * kChunk + row];
...
#pragma unroll
      ...
      // Warp zero updates row 63 only after every owner has completed dQ.
      __barrier_sync_count(1, 128);
      if (threadIdx.x < kStrip) {
        const int key = key_start + threadIdx.x;
...
nanochat_kda_chunk_wy_backward_c64(
  ...
  const __nv_bfloat16* retained_qbar =
      retained_Q + kRetainedVectorElements;
  const __nv_bfloat16* retained_khat =
      retained_qbar + kRetainedVectorElements;
  const __nv_bfloat16* retained_P =
      retained_khat + kRetainedVectorElements;
  ...
  const float* retained_q_inverse =
      retained_beta + kRetainedScalarElements;
  const float* retained_k_inverse =
  ...
  C10_CUDA_CHECK(cudaFuncSetAttribute(
      ...
      kFusedBoundaryRegisterDhSharedBytes));
#endif
  for (int group_id = kGroups; group_id-- > 0;) {
    ...
    nanochat_kda_wy_backward_register_dh_group_c64_kernel<<<
        ...
        reinterpret_cast<const __nv_bfloat16*>(
            kg_group_bf16.data_ptr<at::BFloat16>()),
        retained_prefix, W_group, A_group,
        reinterpret_cast<const __nv_bfloat16*>(
            grad_output.data_ptr<at::BFloat16>()),
        reinterpret_cast<__nv_bfloat16*>(
            dZ_group_bf16.data_ptr<at::BFloat16>()),
    ...
    at::Tensor dqbar_group = at::empty(
        {kGroupRows, kChunk, kDim}, fp32);
    at::Tensor dkhat_group = at::empty_like(dqbar_group);
    at::Tensor dprefix_group = at::empty_like(dqbar_group);
