# SWA + KDA architecture research

This is a PyTorch research fork of nanochat for controlled experiments with
sliding-window attention and recurrent linear attention. The first milestone is
a rigorous full-attention/SWA baseline and evaluation harness; KDA integration
comes next. Upstream origin and pinned revision are recorded in `UPSTREAM.md`.

Quick start on the current DGX Spark environment:
...
Research runs require a clean Git commit and write immutable artifacts under
`runs/<run-id>/`. Discovery is fixed-time; promotion is fixed-token. Read
`AGENTS.md`, `program.md`, and `runbook/index.md` before changing a protocol or
launching an autoresearch loop.

The material below is the inherited nanochat documentation. Shell recipes that
were originally under `runs/` now live under `recipes/nanochat/`; generated
research artifacts exclusively use `runs/`.

# Upstream nanochat documentation
...
Presently, the main focus of upstream development is tuning pretraining. Inspired by modded-nanogpt, nanochat maintains a "GPT-2 speedrun" leaderboard measured by DCLM CORE. The inherited [speedrun recipe](recipes/nanochat/speedrun.sh) records that workflow. The current upstream leaderboard looks as follows:
...
The upstream primary metric is "time to GPT-2" - the wall clock time needed to outperform the GPT-2 (1.6B) CORE metric on an 8XH100 GPU node. The architecture-research protocol in this fork uses the multi-objective contract described above.
...
The inherited end-to-end pipeline is in [recipes/nanochat/speedrun.sh](recipes/nanochat/speedrun.sh), designed for an 8XH100 node:

```bash
bash recipes/nanochat/speedrun.sh
```
...
Two inherited scripts are [scaling_laws.sh](recipes/nanochat/scaling_laws.sh) and [miniseries.sh](recipes/nanochat/miniseries.sh). See [Jan 7 miniseries v1](https://github.com/karpathy/nanochat/discussions/420) for upstream context. The upstream quick-experiment example is:
...
The inherited [runcpu.sh](recipes/nanochat/runcpu.sh) recipe shows a small CPU or Apple Silicon example.
