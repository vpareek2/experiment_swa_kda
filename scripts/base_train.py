...
parser.add_argument("--seed", type=int, default=42, help="model initialization and training RNG seed")
# FP8 training
...
parser.add_argument("--max-training-seconds", type=float, default=-1.0, help="stop after this much steady-state training time; compilation and evaluation are excluded")
# Optimization
...
assert args.num_iterations > 0 or args.target_param_data_ratio > 0 or args.target_flops > 0 or args.max_training_seconds > 0
if args.max_training_seconds > 0 and args.num_iterations <= 0:
    # A large safety ceiling; the wall-clock condition below is authoritative.
    num_iterations = 1_000_000_000
    print0(f"Using steady-state training-time budget: {args.max_training_seconds:.1f}s")
...
total_tokens = None if args.max_training_seconds > 0 else total_batch_size * num_iterations
if total_tokens is None:
    print0("Total training tokens and FLOPs will be measured at the time-budget boundary")
else:
...
def get_lr_multiplier(it):
    if args.max_training_seconds > 0:
        progress = min(total_training_time / args.max_training_seconds, 1.0)
        warmup_ratio = 0.05
        warmdown_start = 1.0 - args.warmdown_ratio
        if progress < warmup_ratio:
            return max((progress + 1e-6) / warmup_ratio, 1.0 / max(args.warmup_steps, 1))
        if progress <= warmdown_start:
            return 1.0
        warmdown_progress = (progress - warmdown_start) / max(args.warmdown_ratio, 1e-12)
        return 1.0 + warmdown_progress * (args.final_lr_frac - 1.0)
...
def get_muon_momentum(it):
    if args.max_training_seconds > 0:
        progress = min(total_training_time / args.max_training_seconds, 1.0)
        if progress < 0.05:
            return 0.85 + (progress / 0.05) * (0.97 - 0.85)
        warmdown_start = 1.0 - args.warmdown_ratio
        if progress >= warmdown_start:
            warmdown_progress = (progress - warmdown_start) / max(args.warmdown_ratio, 1e-12)
            return 0.97 + warmdown_progress * (0.90 - 0.97)
        return 0.97
    warmdown_iters = round(args.warmdown_ratio * num_iterations)
...
while True:
    time_budget_reached = args.max_training_seconds > 0 and total_training_time >= args.max_training_seconds
    last_step = step >= num_iterations or time_budget_reached # final pass performs eval/save only
    ...
    pct_done = (100 * min(total_training_time / args.max_training_seconds, 1.0)
                if args.max_training_seconds > 0 else 100 * step / num_iterations)
    tok_per_sec = int(total_batch_size / dt)
    flops_per_sec = num_flops_per_token * total_batch_size / dt
    mfu = 100 * flops_per_sec / (gpu_peak_flops * ddp_world_size)
    if step > 10:
        total_training_time += dt # only count the time after the first 10 steps
        throughput_samples.append(tok_per_sec)
    # Calculate ETA based on average time per step (excluding first 10 steps)
    steps_done = step - 10
    if steps_done > 0:
        avg_time_per_step = total_training_time / steps_done
        eta_seconds = (max(args.max_training_seconds - total_training_time, 0.0)
                       if args.max_training_seconds > 0 else (num_iterations - step) * avg_time_per_step)
    ...
    if master_process:
        print0("RESEARCH_TRAIN_STEP " + json.dumps({
            "schema_version": 1,
            "step": step,
            "training_tokens": total_batch_size * (step + 1),
...
if master_process:
    sorted_throughput = sorted(throughput_samples)
    median_throughput = sorted_throughput[len(sorted_throughput) // 2] if sorted_throughput else 0
    summary = {
        "schema_version": 1,
        "status": "complete",
        ...
        "seed": args.seed,
        "step": step,
        "training_tokens": total_batch_size * step,
        "training_seconds": total_training_time,
        "val_bpb": val_bpb,
