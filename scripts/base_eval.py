...
def main():
    ...
    parser.add_argument('--output', type=str, default=None, help='write a machine-readable JSON result')
    ...
    if ddp_rank == 0:
        structured_result = {
            ...
            "split_tokens": args.split_tokens if "bpb" in eval_modes else None,
            "max_per_task": args.max_per_task if "core" in eval_modes else None,
        }
        if args.output:
            ...
            with open(temporary_path, "w", encoding="utf-8") as handle:
                json.dump(structured_result, handle, indent=2, sort_keys=True, allow_nan=False)
                handle.write("\n")
            os.replace(temporary_path, output_path)
        print0("RESEARCH_EVAL_RESULT " + json.dumps(structured_result, sort_keys=True, allow_nan=False))
